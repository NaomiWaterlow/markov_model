...
swab_weekly <- data.table("current_week" = NA, 
                          "rsv_current" = NA, 
                          "flu_target" = NA, 
                          "flu_current" = NA, 
                          "rsv_target" = NA, 
...
weeks_of_interest <- c(-3:3)
n_samples <- 5
set.seed(63)
## changes to implement. 
# - for each of the positive swabs, take 5 samples (age-matched), rather 
#   than doing it for the whole thing.
...
for(pos_sample in 1:nrow(firsts_rsv)){
  ...
  target_date <- unlist(firsts_rsv[pos_sample,"rounded"])
  ...
  controls_all <-  main_db[rounded == target_date &
                             age_grp == target_age &
                             rsv == 0,]
  ...
  for(j in 1:length(c(weeks_of_interest))){
    search_week <- weeks_of_interest[j]
    
    # need to remove the weeks that don't exist#
    # e.g. can't look -3 backs if youre cases in the first week of data
    if(!(target_date + (7*search_week)) <= season_dates[1]& !(target_date + (7*search_week)) >= season_dates[2] ){
      # for each individual
      for(k in 1:(1+n_samples)){
        # subset only that individuals data
        temp <- weekly[factor_id ==ids_to_run[k],]
        this_week_flu <- NA
        target_week_rsv <- NA
        # In the current week, what is rsv?
        if(temp[rounded ==target_date,"rsv"] == 0){this_week_rsv <- 0
        } else if(temp[rounded ==target_date,"rsv"] == 1){this_week_rsv <- 1
        }else {
          browser()
          
        }
        # in the target week what is flu
        if(temp[rounded == (target_date + 7*search_week),
                "flu"] == 0){target_week_flu <- 0
        } else if(temp[rounded == (target_date + 7*search_week),
                       "flu"] == 1){target_week_flu <- 1}
        temp2 <- data.frame(target_date, this_week_rsv, target_week_flu,
                            this_week_flu, target_week_rsv, search_week)
        colnames(temp2) <- colnames(swab_weekly)
        
        swab_weekly <- rbind(swab_weekly,temp2 )
      }}
  }
  
}
# when there is no rsv, what proportion of the target week have flu
rsv0 <- swab_weekly[rsv_current == 0, (sum(flu_target)/.N), by = "target_week"]
# when there is rsv, what proportion of the targt week have flu
rsv1 <- swab_weekly[rsv_current == 1, (sum(flu_target)/.N), by = "target_week"]
colnames(rsv0) = colnames(rsv1) = c("target_week", "prop_in_target")
rsv0$infected <- F
rsv1$infected <- T

rsv0$N <- swab_weekly[rsv_current == 0, .N, by = "target_week"]$N
rsv1$N <- swab_weekly[rsv_current == 1, .N, by = "target_week"]$N
...
for(pos_sample in 1:nrow(firsts_flu)){
  ...
  target_date <- unlist(firsts_flu[pos_sample,"rounded"])
  ...
  for(j in 1:length(c(weeks_of_interest))){
    ...
    if(!(target_date + (7*search_week)) <= season_dates[1]& !(target_date + (7*search_week)) >= season_dates[2] ){
      ...
      for(k in 1:(1+n_samples)){
        ...
        target_week_flu <- NA
        # In the current week, what is rsv?
        if(temp[rounded ==target_date,"flu"] == 0){this_week_flu <- 0
        } else if(temp[rounded ==target_date,"flu"] == 1){this_week_flu <- 1}
        # in the target week what is flu
        if(temp[rounded == (target_date + 7*search_week),
                "rsv"] == 0){target_week_rsv <- 0
        } else if(temp[rounded == (target_date + 7*search_week),
                       "rsv"] == 1){target_week_rsv <- 1}
        temp2 <- data.frame(target_date, this_week_rsv, target_week_flu,
                            this_week_flu, target_week_rsv, search_week)
        colnames(temp2) <- colnames(swab_weekly)
        swab_weekly <- rbind(swab_weekly,temp2 )
...
flu0 <- swab_weekly[flu_current == 0, (sum(rsv_target)/.N), by = "target_week"]
flu1 <- swab_weekly[flu_current == 1, (sum(rsv_target)/.N), by = "target_week"]

colnames(flu0) = colnames(flu1) = c("target_week", "prop_in_target")
flu0$infected <- F
flu1$infected <- T

flu0$N <- swab_weekly[flu_current == 0, .N, by = "target_week"]$N
flu1$N <- swab_weekly[flu_current == 1, .N, by = "target_week"]$N
...
together[, upper := (prop_in_target + z*sqrt((prop_in_target * (1-prop_in_target))/N))*100]
together[, lower := (prop_in_target - z*sqrt((prop_in_target * (1-prop_in_target))/N))*100]
together[, percent_in_target := prop_in_target*100]
...
PERCENT <- ggplot(together, aes(x = target_week, y = percent_in_target,fill = infected)) +
  geom_line(aes(colour = infected)) +geom_point(aes(colour = infected)) + 
  geom_ribbon(aes(ymin = lower, ymax = upper),alpha = 0.3) +
  #geom_bar(stat="identity", position = "dodge") +
  facet_grid(.~base_virus ) + 
  theme_linedraw() + 
  labs( x = "Week relative to swab of virus", y = "Percentage infected with other virus", 
...
prop.test(x = c(N_observed_dual, N_expected_dual), n = c(N_swabs, N_swabs))
...
flu1_val <- round(PHIRST_BACKGROUND[indid %in% flu1, mean(age_at_consent)])
...
flu2_val <- round(PHIRST_BACKGROUND[indid %in% flu2, mean(age_at_consent)])
...
flu3_val <- round(PHIRST_BACKGROUND[indid %in% flu3, mean(age_at_consent)])
...
rsv1_val <- round(PHIRST_BACKGROUND[indid %in% rsv1, mean(age_at_consent)])
...
rsv2_val <- round(PHIRST_BACKGROUND[indid %in% rsv2, mean(age_at_consent)])
...
rsv3_val <- round(PHIRST_BACKGROUND[indid %in% rsv3, mean(age_at_consent)])
...
print(paste0("season 1: flu mean age is ", flu1_val, " and  rsv mean age is ", rsv1_val))
print(paste0("range flu is ", flu1_min, " to ", flu1_max))
print(paste0("range rsv is ", rsv1_min, " to ", rsv1_max))
print(paste0("season 2: flu mean age is ", flu2_val, " and  rsv mean age is ", rsv2_val))
print(paste0("range flu is ", flu2_min, " to ", flu2_max))
print(paste0("range rsv is ", rsv2_min, " to ", rsv2_max))
print(paste0("season 3: flu mean age is ", flu3_val, " and  rsv mean age is ", rsv3_val))
